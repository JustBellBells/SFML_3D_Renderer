...
Matrix::Matrix(int width, int height, float frame)
{
    ...
    this->perspectiveMatrix = glm::perspective(glm::radians(45.0f), (float)width / (float)height, 0.1f, 100.0f);
}
...
glm::vec4 Matrix::matrixTranslate(float &x, float &y, float &z)
{
    ...
    translation = glm::rotate(translation, glm::radians(this->frame * 2), glm::vec3(1.f, 1.f, 1.f));
    vec = translation * vec;
